...
SkyBox::SkyBox(GLfloat width, GLfloat height, GLfloat depth, glm::vec3 move, GLfloat size, glm::vec3 color, GLfloat texDensity)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * skyBoxCoord.size(), &(skyBoxCoord[0]), GL_STATIC_DRAW);
...
void SkyBox::draw(const glm::mat4 & modelTrans, GLuint modelLoc) const
{
	...
	glDrawArrays(GL_TRIANGLES, 0, (GLuint)skyBoxCoord.size());
...
void SkyBox::bindTexture(GLuint progID)
{
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, texture0);
...
void SkyBox::addBox(GLfloat width, GLfloat height, GLfloat depth, glm::vec3 move, GLfloat size, glm::vec3 color, GLfloat texDensity)
{
	for (int i = 0; i < _countof(boxMesh) / 8; ++i) {
		...
		skyBoxCoord.push_back(poss.x);
		skyBoxCoord.push_back(poss.y);
		skyBoxCoord.push_back(poss.z);
		skyBoxCoord.push_back(color.x);
		skyBoxCoord.push_back(color.y);
		skyBoxCoord.push_back(color.z);
		skyBoxCoord.push_back(boxMesh[i * 8 + 3] * -1.0f);
		skyBoxCoord.push_back(boxMesh[i * 8 + 4] * -1.0f);
		skyBoxCoord.push_back(boxMesh[i * 8 + 5] * -1.0f);
		skyBoxCoord.push_back(boxMesh[i * 8 + 6] * texDensity);
		skyBoxCoord.push_back(boxMesh[i * 8 + 7] * texDensity);
	}
}
